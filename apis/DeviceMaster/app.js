...
async function getDevice(id) {
	...
	const data = await db.any(
		"SELECT device.*, sa.name as uname, sa.username, sys.name as sysname, count(device.*) OVER() AS full_count FROM devicetypes device JOIN superadmins sa ON device.createdBy= sa.id JOIN systemtypes sys ON device.systemid = sys.id WHERE device.id = $1",
		[device_id]
	);
	return [data, 200];
}
